...
\subsection{HCP Data}
HCP Data is collected from a population of adult twins and their non-twin siblings in the age range of 22 - 35 years. Data from 1200 subjects is available in the ConnectomeDB drepository \footnote{\url{https://db.humanconnectome.org/}}. According to HCP data set, data collected from each individula is denoted by the term ``subject". Each subject contains data belonging to four modalities. They are structural MRI, resting-state fMRI (rfMRI), task fMRI(tfMRI), and diffusion MRI (dMRI)~\cite{HCPData}.
...
Functional data consists of resting state and task based data. Emotion, gambling, language, motor, relational and social are examples of task based fMRI data. Resting state data consists of 2 sets (Rest1 and Rest2) in HCP subjects. Task based fMRI data is used for studying functional brain networks under task performance.
