...
Dice Similarity Coefficient can be used as a statistical validation metric for measuring the reproducibility of magnetic resonance images~\cite{Zou2004}. To measure the similarity of image files in reproducibility study, we used dice similarity coefficient. The coefficient ranges from 0.0 to 1. 1 meaning the images are exactly similiar and 0 meaning there is zero similarity. The equation used for finding the dice coefficient is given in \ref{eq:eq_two}. The algorithm used for finding the dice similarity coefficient is described as Algorithm 2. Consider im1 \& im2, as non-zero voxels from two images.

\begin{equation}
  \label{eq:eq_two}
    Dice\_coefficient = {((im1 + im2) - (im1 - im2))/(im1 + im2)}
\end{equation}
