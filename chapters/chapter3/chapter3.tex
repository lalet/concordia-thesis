...
\Pn{\FNrmse{$image\-1$,$image\-2$}}{
  \begin{algorithmic}
    \STATE image\_1=Convert\_image(image\-1);
    \STATE image\_2=Convert\_image(image\-2);
    \STATE mininmum\_intensity=get the value using FSL from image\_1; \COMMENT{using fslstats command}
    \STATE maximum\_intensity=get the value using FSL from image\_2;  \COMMENT{using fslstats command}
    \STATE \textbf{fslmaths} image\-1 \text{-sub} image\-2 \text{-sqr} diff; \COMMENT{computing the mean square difference using FSL}
    \STATE meandifference=\textbf{fslstats} diff -m; \COMMENT{get the mean square difference}
    \STATE \textbf{NRMSE}=$\sqrt{(meandifference/(max-min))}$; \COMMENT{Normalized root mean square error value}
  \end{algorithmic}
\KwRet NRMSE\;
}
