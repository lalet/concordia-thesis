...
%Add algorithm
\hrulefill
\caption*{\textbf{Algorithm 1:} Algorithm for finding the file differences and metric values}
...
\FOR{each pair of conditions:C1,C2 in conditions}
  \FOR{each file name f in L\textsubscript{S,C}}
    \STATE{n\_differences[f][C1][C2] = 0}
    \STATE{distance[f][C1][C2] = 0}
      \FOR{each subject S in condition C1}
        \STATE{metric[f][C1][C2][S] = 0}
        \STATE{differ = 0}
          \IF{size(f,S,C1)!=size(f,S,C2) $\parallel$ checksum(f,S,C1)!=checksum(f,S,C2)}
            \STATE{metric[f][C1][C2][S]=metric(f,C1,C2,S)}
            \STATE{differ = 1}
          \ENDIF
        \STATE{n\_differences[f][C1][C2] += differ}
        \STATE{metric[f][C1][C2] += metric[f][C1][C2][S]}
      \ENDFOR
  \ENDFOR
\ENDFOR
\end{algorithmic}
...
Dice Similarity Coefficient can be used as a statistical validation metric for measuring the reproducibility of magnetic resonance images~\cite{Zou2004}. To measure the similarity of image files in reproducibility study, we used dice similarity coefficient. The coefficient ranges from 0.0 to 1. 1 meaning the images are exactly similiar and 0 meaning there is zero similarity. The equation used for finding the dice coefficient is given in \ref{eq:eq_two}. The algorithm used for finding the dice similarity coefficient is described as Algorithm 2. Consider I1 \& I2, as non-zero voxels from two images.

\begin{equation}
  \label{eq:eq_two}
    Dice\_coefficient = {((|I1| + |I2|) - (2|I1 - I2|))/(|I1| + |I2|)}
\end{equation}
