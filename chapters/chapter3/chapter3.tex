...
\Pn{\FNrmse{$image\-1$,$image\-2$}}{
  \begin{algorithmic}
    ...
    \STATE mininmum\_intensity=get using fsl from image\_1; \COMMENT{using fslstats command}
    \STATE maximum\_intensity=get using fsl from image\_2;  \COMMENT{using fslstats command}
    \STATE \textbf{fslmaths} image\-1  \-sub image\-2 \-sqr diff; \COMMENT{computing the mean square difference using FSL}
