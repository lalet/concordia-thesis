...
\Pn{\FNrmse{$image\-1$,$image\-2$}}{
  \begin{algorithmic}
    ...
    \STATE \textbf{NRMSE} $\leftarrow$ $\sqrt{(meandifference/(max-min))}$; \COMMENT{Normalized root mean square error value}
  \end{algorithmic}
\KwRet NRMSE\;
}
\caption*{Algorithm for finding NRMSE Value of a pair of images}
\label{alg:algorithm_nrmse}

\subsection{Dice Similarity Coefficient}
Dice Similarity Coefficient can be used as a statistical validation metric for measuring the reproducibility of magnetic resonance images~\cite{Zou2004}. To measure the similarity of image files in reproducibility study, we used dice similarity coefficient. The coefficient ranges from 0.0 to 1. 1 meaning the images are exactly similiar and 0 meaning there is zero similarity.
