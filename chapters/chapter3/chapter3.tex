...
The Docker container containing the HCP Pipeline along with the Boutiques descriptor was deployed on a single server setup as part of our study. With the help of CBRAIN along with the containers and the descriptors, we were able to process multiple HCP subjects at once. Single server was used in order to prevent differences occuring in the files due to differences in the hardware.
\begin{itemize}
